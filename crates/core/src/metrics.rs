...
impl Metrics {
    ...
    pub fn print_summary(&self) {
        ...
        if !data.write_times.is_empty() {
            let avg_write = data.write_times.iter().sum::<Duration>() / data.write_times.len() as u32;
    ...
    pub fn write_throughput_mbps(&self) -> Option<f64> {
        if let Some(avg_time) = self.average_write_time() {
            ...
            if avg_time.as_secs_f64() > 0.0 && bytes_written > 0 {
                let mb_per_sec = (bytes_written as f64) / (1024.0 * 1024.0) / avg_time.as_secs_f64();
                Some(mb_per_sec)
            } else {
                Some(0.0)
            }
...
impl WorkloadMetrics {
    ...
    pub async fn print_summary(&self) {
        ...
        if !data.batch_times.is_empty() {
            let avg_batch_time = data.batch_times.iter().sum::<Duration>() / data.batch_times.len() as u32;
