...
async fn test_multi_backend_comprehensive() -> Result<()> {
        ...
        match batch_result {
            Ok(batch) => {
                ...
                for file_data in &batch {
                    ...
                    assert!(
                        content.contains("Backend test file"),
    ...
    println!(
        "✅ File backend processed {} files in {:?}",
        files_processed, total_time
    );
...
async fn test_auto_tuning_optimization() -> Result<()> {
        ...
        match batch_result {
            Ok(batch) => {
                ...
                for file_data in &batch {
                    assert!(
                        !file_data.is_empty(),
                        "Auto-tuned batch should have valid data"
                    );
                }
            }
    ...
    assert!(
        throughput > 5.0,
        "Auto-tuning should achieve reasonable throughput"
    );
