...
fn test_file_backend_comprehensive_dataloader() {
    ...
    rt.block_on(async {
        ...
        assert!(result.is_ok(), "File backend comprehensive test failed: {:?}", result.err());
    });
}
...
fn test_directio_backend_comprehensive_dataloader() {
    ...
    rt.block_on(async {
        ...
        assert!(result.is_ok(), "DirectIO backend comprehensive test failed: {:?}", result.err());
    });
}
...
fn test_s3_backend_comprehensive_dataloader() {
    ...
    rt.block_on(async {
        ...
        if std::env::var("AWS_ACCESS_KEY_ID").is_ok() && std::env::var("AWS_SECRET_ACCESS_KEY").is_ok() {
            ...
            assert!(result.is_ok(), "S3 backend comprehensive test failed: {:?}", result.err());
        } else {
...
fn test_azure_backend_comprehensive_dataloader() {
    ...
    rt.block_on(async {
        ...
        if std::env::var("AZURE_BLOB_ACCOUNT").is_ok() && std::env::var("AZURE_BLOB_CONTAINER").is_ok() {
            let result = run_backend_comprehensive_test("azure").await;
            assert!(result.is_ok(), "Azure backend comprehensive test failed: {:?}", result.err());
        } else {
...
async fn run_backend_comprehensive_test(backend_type: &str) -> Result<()> {
    println!("🚀 Starting COMPREHENSIVE s3dlio DataLoader test for {} backend...", backend_type.to_uppercase());
    ...
    let (store_uri, temp_dir) = match backend_type {
        "file" => {
            ...
            (format!("file://{}", temp_path.to_string_lossy()), Some(temp_dir))
        }
        "directio" => {
            let temp_dir = TempDir::new()?;
            let temp_path = temp_dir.path();
            ...
            (format!("direct://{}", temp_path.to_string_lossy()), Some(temp_dir))
        }
        "s3" => {
        ...
        _ => return Err(anyhow::anyhow!("Unsupported backend type: {}", backend_type)),
    };
    
    println!("📁 Created {} test files for {} backend", num_files, backend_type.to_uppercase());
...
fn create_text_content(index: usize, size: usize) -> String {
    ...
    let remaining = size.saturating_sub(base.len());
    format!("{}{}", base, "Lorem ipsum dolor sit amet ".repeat(remaining / 26 + 1)[..remaining].to_string())
}
...
async fn run_dataloader_test(store_uri: &str, backend_type: &str, num_files: usize) -> Result<()> {
    println!("🔧 Setting up comprehensive AsyncPoolDataLoader for {} backend...", backend_type.to_uppercase());
    ...
    if file_list.len() < num_files {
        println!("⚠️ Warning: Found {} files, expected {} for {} backend", 
                file_list.len(), num_files, backend_type);
    }
    
    // Create MultiBackendDataset
    let dataset = MultiBackendDataset::from_prefix(store_uri).await?;
    println!("✅ Dataset created with {} files for {} backend", dataset.len(), backend_type.to_uppercase());
    ...
    println!("🚀 Starting comprehensive async batch processing for {} backend...", backend_type.to_uppercase());
    ...
    while let Some(batch_result) = stream.next().await {
        ...
        match batch_result {
            ...
            Err(e) => {
                return Err(anyhow::anyhow!("{} backend batch processing failed: {}", backend_type, e));
            }
        }
    }
    ...
    println!("✅ {} backend comprehensive DataLoader test completed successfully!", backend_type.to_uppercase());
...
fn verify_batch_integrity(batch: &[Vec<u8>], backend_type: &str) -> Result<()> {
    for (i, file_data) in batch.iter().enumerate() {
        ...
        if file_data.is_empty() {
            return Err(anyhow::anyhow!("{} backend returned empty file at index {}", backend_type, i));
        }
        
        if file_data.len() < 100 {
            return Err(anyhow::anyhow!("{} backend file too small: {} bytes at index {}", backend_type, file_data.len(), i));
        }
        ...
        if !has_expected_content {
            println!("⚠️ Debug: Content preview for file {}: {}", i, &first_part[..first_part.len().min(50)]);
            return Err(anyhow::anyhow!("{} backend file content invalid at index {}: no expected patterns found", backend_type, i));
        }
    }
...
fn print_backend_results(backend_type: &str, metrics: &DataLoaderMetrics, total_time: Duration) {
    let avg_batch_time = metrics.batch_times.iter().sum::<Duration>() / metrics.batch_times.len() as u32;
    ...
    println!("\n🎯 {} BACKEND COMPREHENSIVE RESULTS:", backend_type.to_uppercase());
    ...
    println!("\n📋 {} CONTENT TYPE ANALYSIS:", backend_type.to_uppercase());
...
fn verify_backend_performance(backend_type: &str, metrics: &DataLoaderMetrics, total_time: Duration) -> Result<()> {
    let files_per_second = metrics.total_files as f64 / total_time.as_secs_f64();
    ...
    if files_per_second < min_throughput {
        return Err(anyhow::anyhow!(
            ...
            backend_type, files_per_second, min_throughput
        ));
    }
    ...
    if metrics.total_files < 50 {
        return Err(anyhow::anyhow!(
            ...
            backend_type, metrics.total_files
        ));
    }
    ...
    if metrics.content_types.len() < 3 {
        return Err(anyhow::anyhow!(
            ...
            backend_type, metrics.content_types.len()
        ));
    }
