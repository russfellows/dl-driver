...
#[test]
fn test_async_pool_dataloader_comprehensive() {
...
async fn run_comprehensive_test() -> Result<()> {
    ...
    println!(
        "📁 Creating {} test files of {} bytes each...",
    ...
    for i in 0..num_files {
        ...
        let content = format!(
            "Test file {} with data: {}",
            i,
            "x".repeat(file_size_bytes - 50)
        );
        std::fs::write(&file_path, &content)?;
        expected_file_contents.insert(
    ...
    println!(
        "📤 Verifying {} files are accessible via ObjectStore...",
        num_files
    );
    ...
    assert!(
        dataset.len() >= num_files,
        "Dataset should contain at least {} files",
        num_files
    );
        ...
        match batch_result {
            Ok(batch) => {
                ...
                for file_data in &batch {
                    ...
                    assert!(
                        file_data.len() >= 50,
                        "File should have minimum expected content"
                    );

                    // Verify it's actual test data we created
                    let content_str = String::from_utf8_lossy(file_data);
                    assert!(
                        content_str.starts_with("Test file"),
                        "File should contain expected test data"
                    );
                }
...
#[test]
fn test_dynamic_batching_eliminates_head_latency() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async {
        let result = run_head_latency_test().await;
        assert!(
            result.is_ok(),
            "Head latency test failed: {:?}",
            result.err()
        );
    });
}

async fn run_head_latency_test() -> Result<()> {
    ...
    while let Some(batch_result) = stream.next().await {
        ...
        match batch_result {
            Ok(batch) => {
                println!(
                    "📦 Batch arrived after {:?} with {} files",
                    interval,
                    batch.len()
                );
            }
    ...
    let long_waits = batch_intervals
        .iter()
