...
async fn test_minimal_dlio_config_parsing() -> Result<()> {
    ...
    assert_eq!(dlio_config.model.as_ref().unwrap().name.as_deref(), Some("my_workload"));
    ...
    assert_eq!(dlio_config.dataset.data_folder, "file:///mnt/vast1/dlio_minimal_data");
...
async fn test_unet3d_dlio_config_parsing() -> Result<()> {
    ...
    assert_eq!(dlio_config.model.as_ref().unwrap().name.as_deref(), Some("unet3d_workload"));
    assert_eq!(dlio_config.model.as_ref().unwrap().model_size, Some(499153191));
...
async fn test_bert_dlio_config_parsing() -> Result<()> {
    ...
    // Validate model configuration
    assert_eq!(dlio_config.model.as_ref().unwrap().name.as_deref(), Some("bert_workload"));
    assert_eq!(dlio_config.model.as_ref().unwrap().model_size, Some(110176256));
    ...
    assert_eq!(dlio_config.dataset.data_folder, "s3://dlio-benchmark/bert/data");
    ...
    assert_eq!(dlio_config.reader.file_access_type.as_deref(), Some("multi_threaded"));
    ...
    assert_eq!(checkpointing.checkpoint_folder.as_deref(), Some("s3://dlio-benchmark/bert/checkpoints"));
...
async fn test_resnet_dlio_config_parsing() -> Result<()> {
    ...
    // Validate model configuration
    assert_eq!(dlio_config.model.as_ref().unwrap().name.as_deref(), Some("resnet50_workload"));
    assert_eq!(dlio_config.model.as_ref().unwrap().model_size, Some(25583592));
    
    // Validate Azure backend
    assert_eq!(dlio_config.dataset.data_folder, "az://mlcommons/resnet/imagenet");
    ...
    assert_eq!(dlio_config.reader.file_access_type.as_deref(), Some("direct_io"));
...
async fn test_cosmoflow_dlio_config_parsing() -> Result<()> {
    ...
    // Validate model configuration
    assert_eq!(dlio_config.model.as_ref().unwrap().name.as_deref(), Some("cosmoflow_workload"));
    assert_eq!(dlio_config.model.as_ref().unwrap().model_size, Some(402568796));
    
    // Validate DirectIO backend for HPC
    assert_eq!(dlio_config.dataset.data_folder, "direct:///lustre/cosmoflow/data");
...
"#, data_path.to_string_lossy());
    ...
    assert!(pool_config.pool_size > 0);
    assert!(pool_config.readahead_batches > 0);
    println!("✅ PoolConfig validated: pool_size={}, readahead_batches={}", 
    ...
    println!("✅ Object store created for URI: {}", dlio_config.data_folder_uri());
    
    // Validate that the store was created successfully (using it prevents unused warning)
    println!("✅ Object store connectivity validated: {:?}", std::ptr::addr_of!(store));
