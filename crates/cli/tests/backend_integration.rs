...
async fn test_file_backend() -> Result<()> {
    ...
    println!(
        "✅ File backend test passed - {} files, {} bytes",
...
async fn test_s3_backend_conditional() -> Result<()> {
    ...
    match runner.run().await {
        Ok(_) => {
            ...
            assert!(metrics.files_processed() > 0, "Should process files");
            println!(
                "✅ S3 backend test passed - {} files",
                metrics.files_processed()
            );
        }
        Err(e) => {
            println!(
                "⚠️  S3 test failed (expected without proper credentials): {}",
                e
            );
...
async fn test_azure_backend_conditional() -> Result<()> {
    ...
    match runner.run().await {
        Ok(_) => {
            ...
            assert!(metrics.files_processed() > 0, "Should process files");
            println!(
                "✅ Azure backend test passed - {} files",
                metrics.files_processed()
            );
        }
        Err(e) => {
            println!(
                "⚠️  Azure test failed (expected without proper credentials): {}",
                e
            );
