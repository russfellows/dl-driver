...
async fn run_large_scale_test() -> Result<()> {
    ...
    assert!(
        file_list.len() >= num_files,
        "Should have at least {} files, found {}",
        num_files,
        file_list.len()
    );
    ...
    println!(
        "   - Batch size: {} (large batches for efficiency)",
    ...
    let mb_per_second =
        (total_bytes_processed as f64 / 1024.0 / 1024.0) / total_processing_time.as_secs_f64();
    ...
    assert!(
        ...
        "Should have processed at least {} files, got {}",
        num_files,
    ...
    assert!(
        ...
        "Should have processed actual data"
    );

    // Verify high-performance characteristics
    assert!(
        files_per_second > 100.0,
        "Large-scale processing should achieve high throughput (got {:.2} files/sec)",
        files_per_second
    );
    assert!(
        mb_per_second > 1.0,
        "Should achieve reasonable data throughput (got {:.2} MB/sec)",
        mb_per_second
    );
    assert!(
    ...
    assert!(
        content_type_counts.len() >= 3,
        "Should have processed diverse content types"
    );

    println!("\n✅ LARGE-SCALE test completed successfully!");
    println!(
        "✅ Processed {} files ({:.2} MB) in {:?} with {:.2} files/sec throughput",
        total_files_processed,
        total_bytes_processed as f64 / 1024.0 / 1024.0,
...
async fn run_concurrent_test() -> Result<()> {
    ...
    println!(
        "✅ Created two datasets: {} and {} files",
        dataset1.len(),
        dataset2.len()
    );
        ...
        match batch_result {
            Ok(batch) => {
                ...
                for file_data in &batch {
                    let content = String::from_utf8_lossy(file_data);
                    assert!(
                        content.contains("VALIDATION_DATA"),
                        "Should be validation data"
                    );
                }
            }
    ...
    assert!(
        training_files >= num_files_per_dataset,
        "Should process all training files"
    );
    assert!(
        validation_files >= num_files_per_dataset,
        "Should process all validation files"
    );
    assert!(
        total_time < Duration::from_secs(10),
        "Multi-dataset processing should be efficient"
    );
