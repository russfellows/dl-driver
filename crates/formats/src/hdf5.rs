...
impl StreamingFormat for Hdf5Format {
    fn generate_bytes(&self, _filename: &str) -> Result<Vec<u8>> {
        ...
        let mut buffer = Vec::with_capacity(
            16 + dataset_name_bytes.len() + self.shape.len() * 4 + element_count * 4,
        );
    ...
    fn read_from_bytes(&self, data: &[u8]) -> Result<()> {
        ...
        if name != self.dataset_name {
            anyhow::bail!(
                "HDF5 dataset name mismatch: expected '{}', got '{}'",
                self.dataset_name,
                name
            );
        }
        ...
        if shape != self.shape {
            anyhow::bail!(
                "HDF5 shape mismatch: expected {:?}, got {:?}",
                self.shape,
                shape
            );
        }
        ...
        if actual_data_size != expected_data_size {
            anyhow::bail!(
                "HDF5 data size mismatch: expected {}, got {}",
                expected_data_size,
                actual_data_size
            );
        }
