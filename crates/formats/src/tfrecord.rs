...
impl TfRecordFormat {
    ...
    fn create_tf_example(&self, record_index: usize) -> Result<Vec<u8>> {
        ...
        let float_values: Vec<f32> = base_data.chunks_exact(4)
            .map(|chunk| {
...
impl Format for TfRecordFormat {
    ...
    fn read(&self, path: &Path) -> Result<()> {
        ...
        while records_read < self.num_records {
            ...
            reader.read_exact(&mut length_crc_bytes)
                .with_context(|| format!("Failed to read length CRC for record {}", records_read))?;
    ...
    fn read_from_bytes(&self, data: &[u8]) -> Result<()> {
        ...
        while records_read < self.num_records && offset < data.len() {
            ...
            if offset + 8 > data.len() {
                anyhow::bail!("TFRecord: insufficient data for length at record {}", records_read);
            }
            ...
            if offset + 4 > data.len() {
                anyhow::bail!("TFRecord: insufficient data for length CRC at record {}", records_read);
            }
            
            let length_bytes = [
                data[offset - 8], data[offset - 7], data[offset - 6], data[offset - 5],
            ...
            if offset + length > data.len() {
                anyhow::bail!("TFRecord: insufficient data for record {} data", records_read);
            }
            ...
            if offset + 4 > data.len() {
                anyhow::bail!("TFRecord: insufficient data for data CRC at record {}", records_read);
            }
